...
tbge_map_t* map_init()
{
    ...
    if (!map) {
        ...
        SET_MAP_STATUS(MAP_STATUS_INTERNAL_ERROR);
        return NULL;
    }
    ...
    SET_MAP_STATUS(MAP_STATUS_OK);
    return map;
}

int map_add(tbge_map_t* map, tbge_node_h* node)
{
    if (!map || !node) EXIT_WITH_MAP_STATUS(MAP_STATUS_INTERNAL_ERROR);
    ...
    if (map->count >= map->capacity) {
        ...
        if (!new_nodes) {
            ...
            EXIT_WITH_MAP_STATUS(MAP_STATUS_INTERNAL_ERROR);
        }
        map->nodes = new_nodes;
    ...
    EXIT_WITH_MAP_STATUS(MAP_STATUS_OK);
}
...
int map_remove(tbge_map_t* map, int id)
{
    ...
    EXIT_WITH_MAP_STATUS(MAP_STATUS_NODE_NOT_FOUND);
}
...
int map_change_node(tbge_map_t* map, int target_node_id)
{
    ...
    if(current_node == NULL) EXIT_WITH_MAP_STATUS(MAP_STATUS_INTERNAL_ERROR);
    tbge_node_h* target_node = map_search_node(map, target_node_id);
    if(target_node == NULL) EXIT_WITH_MAP_STATUS(MAP_STATUS_NODE_NOT_FOUND);

    if(current_node->locked) EXIT_WITH_MAP_STATUS(MAP_STATUS_CURRENT_NODE_LOCKED);
    if(!node_connected(current_node, target_node_id)) EXIT_WITH_MAP_STATUS(MAP_STATUS_NO_ROUTE);
    if(target_node->locked) EXIT_WITH_MAP_STATUS(MAP_STATUS_TARGET_NODE_LOCKED);
...
void map_log(tbge_map_t* map)
{
    ...
    case MAP_STATUS_OK:
        ...
        break;
    default:
        MAP_LOG_ERR("Unknown Error: %d", map_status_code);
        break;
    }
}
...
int map_lock(tbge_map_t* map, int node_id)
{
    ...
    if(!n) EXIT_WITH_MAP_STATUS(MAP_STATUS_NODE_NOT_FOUND);
...
const char* map_current_node(tbge_map_t* map)
{
    ...
    if(!n) {
        SET_MAP_STATUS(MAP_STATUS_NODE_NOT_FOUND);
        return NULL;
    }
